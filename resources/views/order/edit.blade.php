            ...
            <div class="animated fadeIn">
                ...
                <div class="card">
                    <div class="card-header">
                        <div class="pull-left">
                            <strong>Edit Data Order</strong>
                        </div>
                        <div class="pull-right">
                            <a href="{{ url('order/add') }}" class="btn btn-secondary btn-sm">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-md-4 offset-md-4">
                                <form action="{{ url('order/'. $order->id_order) }}" method="post">
                                    @method('PATCH')
                                    @csrf
                                    <div class="form-group">
                                        <label>ID Order</label>
                                        <input type="varchar" id="id_order" name="id_order" class="form-control" value="{{$order -> id_keluar }}" autofocus required>
                                    </div>
                                    <div class="form-group">
                                        <label>No. Order</label>
                                        <input type="varchar" id="no_order" name="kode_produk" class="form-control" value="{{$order -> no_order }}" autofocus required>
                                    </div>
                                    <div class="form-group">
                                        <label>Waktu Order</label>
                                        <input type="datetime-local" name="tanggal_order" class="form-control" value="{{$order  -> tanggal_order  }}" autofocus required>
                                    </div>
                                    <div class="form-group">
                                        <label>Kode Produk</label>
                                        <input type="varchar" id="kode_produk" name="kode_produk" class="form-control" value="{{$order  -> kode_produk }}" autofocus required>
                                    </div>
                                    <div class="form-group">
                                        <label>Jumlah Order </label>
                                        <input type="number" name="jumlah_order " class="form-control" value="{{$order  -> jumlah_order  }}" autofocus required>
                                    </div>
